...
#
#
# def null_step_5(matrix: np.ndarray) -> np.ndarray: #Set every fifth element of the matrix to zero.
#     matrix.flat[::5] = 0
#     return matrix
#
#
# def null_step_diagonal(matrix: np.ndarray) -> np.ndarray: #Set the main diagonal of the matrix to zero.
#     np.fill_diagonal(matrix, 0)
#     return matrix
#
#
# def fill_gold(matrix: np.ndarray, gold_value: float) -> np.ndarray: # Replace all zeros in the matrix with a specified gold_value.
#     matrix[matrix == 0] = gold_value
#     return matrix
#
#
# def plot_histogram(matrix, title: str): # Display a histogram of the matrix values
...
#
#
# def plot_matrix(matrix: np.ndarray): #Display the matrix as an image.
#     plt.imshow(matrix, cmap='viridis')
#     plt.colorbar()
#     plt.show()
#
#
# def find_median(matrix: np.ndarray) -> ndarray: #Calculate the median value of the matrix.
#     return np.median(matrix)
#
#
# def find_average(matrix: ndarray) -> ndarray: #Calculate the average (mean) value of the matrix.
#     return np.mean(matrix)
#
#
# def get_gold_value(mean_value: float, average_value: float) -> float: #Calculate the average of the mean and average values.
#     return (mean_value + average_value) / 2
#
#
# def find_nearest_value(matrix: ndarray, value: float) -> float: #Find the value in the matrix that is closest to a given value.
#     diff = np.abs(matrix - value)
#     index = np.unravel_index(diff.argmin(), diff.shape)
#     return matrix[index]
#
#
# def generate_matrix(rows: int, cols: int) -> ndarray: #Generate a random matrix with integer values between 0 and 100.
...
def load_image(filepath: str) -> np.ndarray: #Load an image from a given filepath and ensure its size is 600x600 or larger.
...
def show_image(image: np.ndarray, title: str = "", cmap: str = None): #Display an image with an optional title and colormap.
...
def adjust_contrast(img: Image.Image, level: int) -> Image.Image: #Adjust the contrast of an image based on the provided level.
    if not 1 <= level <= 10:
...
def numpy_to_pil(image_np: np.ndarray) -> Image.Image: #Convert a numpy image array to a PIL Image object.
    return Image.fromarray((image_np * 255).astype(np.uint8))


def convert_to_grayscale(img: np.ndarray) -> np.ndarray: #Convert a color image to grayscale.
    return np.dot(img[..., :3], [0.299, 0.587, 0.114])


def split_into_channels(img: np.ndarray): #Split the image into its RGB channels and display them.
...
def edge_detection(img_gray: np.ndarray) -> np.ndarray: #Apply edge detection to a grayscale image.
...
def blur_image(img: np.ndarray, sigma: int = 10) -> np.ndarray: # Apply Gaussian blur to an image.
